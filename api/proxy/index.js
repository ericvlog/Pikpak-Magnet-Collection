...
// 部署后地址: https://你的项目�?vercel.app/api/proxy?url=...
// 改进：修复登入失败问题，对齐本地 Node.js 代理的行�?
...
export default async function handler(req, res) {
  ...
  try {
    // 关键修复：使�?stream 而不�?buffer 中转，避�?Content-Type 丢失
    ...
    // 关键修复：设置正确的 Content-Type（如�?PikPak 返回了的话）
    const contentType = resp.headers.get('content-type');
    if (contentType) {
      res.setHeader('Content-Type', contentType);
    }

    // 关键修复：读�?body 并发送（使用 pipe 方式的替代方案）
